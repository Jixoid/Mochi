...
namespace mochi::rhi::vulkan
{
  ...
  extern "C" fun MochiRHI_MakeAllocator(rhi::Device &device) -> Allocator* {
    return new VK_Allocator(device);
  }



  VK_Allocator::VK_Allocator(rhi::Device &device)
    : rhi::Allocator(device)
  {
    VmaAllocatorCreateInfo allocatorInfo = {};
    allocatorInfo.vulkanApiVersion = VK_API_VERSION_1_4;
    auto& vk_dmng = static_cast<VK_Device&>(device);
  ...
  ) -> sptr<Buffer>
  {
    ...
    if (create & rhi::AllocationCreate::Mapped)
      e_addr = vma_alloc_info.pMappedData;

    
    return make_sptr(new VK_Buffer(m_device, vma_allocator, e_buffer, e_allocation, e_addr, size));
  }
  
  
  fun VK_Allocator::allocImage2(
  ...
  ) -> sptr<Image2>
  {
    ...
    return make_sptr(new VK_Image2(m_device, vma_allocator, e_image, e_allocation, ext));
  }


  fun VK_Allocator::allocSampler2(
  ...
  ) -> sptr<Sampler2>
  {
    ...
    auto e_sampler = vk::raii::Sampler(static_cast<VK_Device&>(m_device).get(), sampler_info);


    return make_sptr(new VK_Sampler2(m_device, std::move(e_sampler)));
  }
