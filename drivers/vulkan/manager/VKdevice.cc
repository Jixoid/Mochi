...
namespace mochi::rhi::vulkan
{
  ...
  extern "C" fun MochiRHI_MakeDevice(std::string_view appName, std::array<u16, 4> appVer) -> rhi::Device* {
    return new rhi::vulkan::VK_Device(appName, appVer);
  }


  VK_Device::VK_Device(std::string_view appName, std::array<u16, 4> appVer)
    : rhi::Device()
    , vk_ctx(), vk_inst(nil)
  ...
  fun VK_Device::f_is_suitable(vk::raii::PhysicalDevice phys_dev) -> bool {
  ...
  fun VK_Device::getMainBuffer(u32 count) -> vk::raii::CommandBuffers {
  ...
  fun VK_Device::initDescriptorHeap(rhi::Allocator &alloc_mgr) -> void {
