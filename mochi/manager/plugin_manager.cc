...
namespace mochi::mng
{
  ...
  PluginManger::PluginManger(Core &core): m_core(core) {}
  ...
  fun PluginManger::hook(this PluginManger& self, IPlugin* inst, HookThing thing, PluginFunc func) -> bool {
  ...
  fun PluginManger::dispatch(HookThing thing) -> void {
    for (const auto &x: m_hooks[static_cast<u64>(thing)])
      x.second(x.first);
  }


  fun PluginManger::load_all_plugin() -> void {
  ...
  fun PluginManger::load_plugin(std::string_view _path) -> bool {
  ...
  fun PluginManger::unload_plugin(std::string_view name) -> void {
