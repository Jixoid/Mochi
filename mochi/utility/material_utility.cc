...
namespace mochi::utility
{
  ...
  fun MaterialUtility::getMaterialDesc(rhi::RenderTarget &target, MaterialProps props) -> sptr<MaterialDesc> {
    // Info Pipeline
    ...
    sptr<rhi::Shader> vert, frag;

    if (auto x = m_hmng.loadCache(sign_v); !x.empty())
      vert = rhi::Shader::make(m_dmng, rhi::ShaderStage::Vertex, x);
    else {
      auto tmp = m_smng.compileGLSL(sign_v, rhi::ShaderStage::Vertex, vfs::open_map(ShSource[props.method]+".vert")->span(), Macros);
      m_hmng.saveCache(sign_v, tmp);
      vert = rhi::Shader::make(m_dmng, rhi::ShaderStage::Vertex, tmp);
    }


    if (auto x = m_hmng.loadCache(sign_p); !x.empty())
      frag = rhi::Shader::make(m_dmng, rhi::ShaderStage::Pixel, x);
    else {
      auto tmp = m_smng.compileGLSL(sign_p, rhi::ShaderStage::Pixel, vfs::open_map(ShSource[props.method]+".frag")->span(), Macros);
      m_hmng.saveCache(sign_p, tmp);
      frag = rhi::Shader::make(m_dmng, rhi::ShaderStage::Pixel, tmp);
    }
